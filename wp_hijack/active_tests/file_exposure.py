...
# Paths we consider HIGH severity if found
_HIGH_SEVERITY = {"/wp-config.php", "/.env", "/.git/HEAD", "/.htpasswd", "/database.sql", "/db.sql"}

# Phrases that indicate a soft-404 / custom error page (case-insensitive)
...
# Content signatures that CONFIRM a real file (not an error page)
...
def _is_soft_404(body: str, body_len: int, baseline_size: int) -> bool:
    ...
    if baseline_size > 0:
        ...
        if delta <= max(60, baseline_size * 0.15):
            return True  # same size as baseline 404 page
...
def _has_real_content(path: str, body: str) -> bool:
    ...
    if sig is None:
        return True   # no signature rule — allow
...
async def check_file_exposure(http, base_url: str) -> list[ExposedFile]:
    ...
    # ── Step 1: Baseline — fetch a guaranteed-nonexistent path ────────────────
    ...
    try:
        ...
        if br.status_code == 200:
            # Server returns 200 for everything (soft-404 server)
    ...
    async def _probe(path: str) -> ExposedFile | None:
        try:
            ...
            # ── Content-signature gate for known files ────────────────────
