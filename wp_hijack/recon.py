...
class ReconInfo:
    ...
    # Raw home HTML (reused by later scanner phases)
    home_html: str = ""
    # Set when IP resolution fails
...
async def run_recon(http_client: Any, target: str) -> ReconInfo:
    ...
    # ── 1. IP resolution (sync, run in thread) ───────────────────────────────
    try:
    ...
    try:
        ...
        info.tech_stack = list(dict.fromkeys(tech))  # dedup, preserve order
    except Exception:
        pass

    # ── 4. GeoIP via ipinfo.io (free, no key required) ───────────────────────
    if info.ip:
        try:
