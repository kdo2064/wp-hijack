...
class LoginSecurityResult:
    ...
    username_oracle: bool = False    # Error message reveals valid usernames
    no_rate_limit: bool = False      # No lockout after N failed attempts
...
async def test_login_security(http, base_url: str) -> LoginSecurityResult:
    ...
    if result.login_accessible:
        try:
            ...
            # No 429/302 to lockout page → no rate limiting detected
    ...
    # ── wp-admin accessible without auth ─────────────────────────────────────
    try:
