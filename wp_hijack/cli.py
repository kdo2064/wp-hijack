...
# ── Windows UTF-8 fix — must run before Rich/typer are imported ───────────────
import sys as _sys
...
def _ai_analyze_prompt(results: dict, cfg: dict) -> None:
    ...
    if not ai_cfg.get("enabled", True):
        return  # AI disabled in config, skip prompt
...
def _do_single_scan(
    ...
    *,
    ...
    # ── AI interactive prompt (after results, before reports) ──────────────────
    if ask_ai:
    ...
    # Determine highest severity for summary
...
def scan(
    ...
    # Validate: must provide either a target URL or a --file
    ...
    for idx, tgt in enumerate(targets, 1):
        ...
        # In bulk mode: auto-skip AI prompt unless --yes-ai
        _ask_ai = False if no_ai_prompt else (True if yes_ai else (not _is_bulk))
        if yes_ai:
            # Force AI enabled and skip interactive prompt by pre-running it
            _ask_ai = False
...
def model_select(
    ...
    # Non-interactive if --name given
    if name:
        chosen = name if name in model_names else None
        # fuzzy: match on base name without tag
        if not chosen:
...
def _run_chat_session(results: dict, cfg: dict) -> None:
    ...
    async def _stream_answer(q: str) -> None:
        console.print(f"[bold {ACCENT_BLUE}]AI ›[/] ", end="")
        async for chunk in session.ask_stream(q):
            console.print(chunk, end="", markup=False)
        console.print("\n")

    while True:
        try:
            question = console.input(f"[bold {BRAND_GREEN}]You ›[/] ").strip()
        except (EOFError, KeyboardInterrupt):
            break
        if not question:
            continue
        if question.lower() in ("exit", "quit", "q"):
            ...
            break
        _asyncio.run(_stream_answer(question))


# ── chat command (standalone) ─────────────────────────────────────────────────
@app.command()
def chat_report(
    ...
    # Reconstruct a minimal scan_results dict from JSON report
    scan_results = {
        "target":    raw.get("target", "unknown"),
        "cms_info":  _DictProxy(raw.get("cms", {})),
        "waf":       _DictProxy(raw.get("waf", {})),
        "confirmed": [],   # simplified — full reconstruction not needed for chat
        "users":     raw.get("users", []),
        "exposed":   raw.get("exposed_files", []),
        "ai_summary": raw.get("ai_summary"),
        "risk_score": raw.get("risk_score"),
    }
    # Build simple confirmed finding proxies
