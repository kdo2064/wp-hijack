...
from .pipeline       import PotentialFinding, VulnStatus, ExploitCode, confirm_batch
...
class Scanner:
    ...
    async def scan(self, target: str) -> dict[str, Any]:
        ...
        async with AsyncHTTPClient(self.cfg) as http:
            ...
            # Reuse home HTML from recon (already fetched in Phase 0)
            ...
            # ─── Phase 3: Plugin/Theme/User Enumeration ──────────────────────────
            ...
            # ─── Phase 6: Vulnerability Matching ─────────────────────────────────
            import json as _json
            potential_findings: list[PotentialFinding] = []
            # deduplicate key: "cve::component"
            ...
            # ── 6b: WPVulnerability.net live API ─────────────────────────────────
            ...
            if conf_cfg.get("run_confirmations", True) and potential_findings:
                with console.status("[scan.phase]Confirming vulnerabilities...[/]", spinner="dots"):
            ...
            console.print(f"  [scan.done]✔[/]  Confirmed: [bold #00FF41]{c_count}[/] / {len(confirmed)} total")
            results["confirmed"] = confirmed

            # ─── Phase 8: AI Exploit Generation ──────────────────────────────────
            if is_ai_enabled(self.cfg):
                    ...
                    with console.status("[scan.phase]AI: Filtering false positives...[/]", spinner="dots"):
                        try:
                            ...
                            for cf in confirmed:
                                ...
                                cf.finding._fp_verdict = verdict  # type: ignore[attr-defined]
                ...
                # ── 8b: Exploit Generation ────────────────────────────────────
                targets_for_exploit = [
                    cf for cf in confirmed
                    if cf.finding.severity in auto_sev and exploit_cfg.get("auto_generate", True)
                ]

                if targets_for_exploit:
                    with console.status(f"[scan.phase]AI: Generating exploits for {len(targets_for_exploit)} finding(s)...[/]", spinner="dots"):
                        for cf in targets_for_exploit:
                            try:
                                exploit = await gen.generate(cf.finding, cf.confirmation, self._target)
                                cf.exploit = exploit
                            except Exception as exc:
                                console.print(f"  [scan.warn]⚠ Exploit generation failed for {cf.finding.cve}: {exc}[/]")
                    console.print(f"  [scan.done]✔[/]  Exploits generated for [bold white]{len(targets_for_exploit)}[/] finding(s)")

                # ── 8c: CVE Plain-English Explanations ───────────────────────
                    ...
                    with console.status(f"[scan.phase]AI: Explaining {len(explain_targets)} CVE(s)...[/]", spinner="dots"):
                        for cf in explain_targets:
                            try:
                                ...
                                cf.finding._cve_explain = explanation  # type: ignore[attr-defined]
                            except Exception as exc:
                ...
                if waf.detected and targets_for_exploit:
                    with console.status(f"[scan.phase]AI: Generating WAF bypass for {waf.name}...[/]", spinner="dots"):
                        try:
                            # Generate bypass for the highest-severity confirmed finding
                            ...
                            top_cf.finding._waf_bypass = bypass_text  # type: ignore[attr-defined]
                ...
                # ── 8e: AI Executive Summary + Risk Score ─────────────────────
                if all_findings_flat:
                    try:
                    ...
                    # No findings — still generate a clean-site executive summary
                    try:
