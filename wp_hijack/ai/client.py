...
def _resolve_ai_params(config: dict[str, Any]) -> dict:
    ...
    # Ollama / local OpenAI-compat servers don't need a real API key.
    # Replace any placeholder value so the request doesn't get rejected.
...
# ── Universal ask_stream() interface ──────────────────────────────────────────
async def ask_stream(
    ...
    *,
    ...
    if provider in ("openai", "ollama", "openai-compat"):
        ...
        # Gemini and unknown providers: fall back to single call
